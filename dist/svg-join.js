...
__export(exports_string_decoder, {
  ...
  StringDecoder: () => {
    {
      return export_StringDecoder;
    }
  }
});
var Tt2, Z, Nt, St, Rt, _t, R, Lt, kt, tt2, rt2, Bt, mt, At2, Ut2, export_StringDecoder, export_default;
var init_string_decoder = __esm(() => {
  ...
  kt = (e, t, r) => (r = e != null ? Tt2(Rt(e)) : {}, Lt(t || !e || !e.__esModule ? Z(r, "default", { value: e, enumerable: true }) : r, e));
  tt2 = R((k2) => {
    k2.byteLength = Mt;
    k2.toByteArray = Pt;
    k2.fromByteArray = Gt;
...
var require_xmldoc = __commonJS((exports, module) => {
  (function() {
    var sax;
    if (typeof module !== "undefined" && exports && !global.xmldocAssumeBrowser) {
      sax = require_sax();
    } else {
      sax = this.sax;
      if (!sax) {
...
var require_package = __commonJS((exports, module) => {
  module.exports = {
    name: "svg-join",
    version: "1.2.1",
    description: "Join svg files in symbol collection.",
    main: "svg-join.js",
    scripts: {
      test: "echo \"Error: no test specified\" && exit 1",
      upgi: "yarn upgrade-interactive --latest"
    },
    author: "Klimentiy Yudintsev",
    bin: {
      "svg-join": "./bin/svg-join"
    },
    dependencies: {
      "@katsstuff/bun-compile-ts": "^1.0.2",
      "fast-glob": "^3.2.11",
      xmldoc: "^1.1.2",
      yargs: "^17.4.1"
    },
    ...
    repository: {
      type: "git",
      url: "git+https://github.com/Climenty/svg-join.git"
    },
    license: "MIT",
    devDependencies: {
      "@types/bun": "latest"
...
// svg-join.ts
var import_xmldoc = __toESM(require_xmldoc(), 1);
var errOut = function(fname, message) {
  console.error(`File: ${fname}\n${message}`);
};
var lookslike = function(src, trg, filter = []) {
  if (filter.length === 0)
    filter = Object.keys(src);
  return filter.every((x) => src[x] && src[x] === trg[x]);
};
var equals = function(src, trg) {
  const filter = Object.keys(src);
  return filter.length === Object.keys(trg).length && filter.every((x) => src[x] === trg[x]);
};
var deletelike = function(obj, filter) {
  if (!Array.isArray(filter))
    filter = Object.keys(filter);
  filter.forEach((x) => delete obj[x]);
};
var CSS_escape = function(str) {
  return str.replace(/([!#$%&()*+,.\\/;<=>?@[\]^`{|}~])/g, "\\$1").replace(/:/g, "\\3A ");
};
var style_keys = function(str) {
  let items = str.match(/\s?[\w-]+:.*?(;|$)/g);
  if (items === null)
    return [];
  return items.map((x) => x.split(":")[0].trim());
};
var style_format = function(names, attr) {
  let pad = "  ";
  let head = "." + names.join(",\n.") + " {\n";
  let body = Object.keys(attr).map((x) => `${pad}${x}: ${attr[x]};\n`).join("");
  return head + body + "}\n";
};
var wipe_style_format = function(style, attr) {
  return style_format(style.map((x) => {
    deletelike(x.attr, attr);
    return x.name;
  }), attr);
};
var addPX = (x) => isFinite(x) ? x + "px" : x;
var parseUnit = (value = "") => value.replace(/[\d.\s]/g, "");
async function svg_join_default(args) {
  console.log(`SVG-Join ${require_package().version} Join svg files in symbol collection.`);
  const argv = {
    sources: args.sources,
    prefix: args.prefix ?? "svg_",
...
`;
  ...
  try {
    ...
    for (const fname of list) {
      ...
      const body = fname;
      try {
        const doc = new import_xmldoc.XmlDocument(body);
        ...
        if (argv.mono) {
          const styled_children = doc.children.filter((x) => {
            ...
            if (x.attr.style)
              keys = keys.concat(style_keys(x.attr.style));
